...
Nginx官方没有的功能，开源开发者定制开发一些功能，把代码公布出来，可以通过**编译加载第三方模块**的方式，**使用新的功能**。（github上查找nginx module）

第三方模块网址：https://www.nginx.com/resources/wiki/modules

## 1、编译安装第三方模块

fancyindex（类似于aotoindex目录升级版）
...
shell > cd /root/nginx-1.16.0
shell > ./configure  --prefix=/usr/local/nginx --user=www --group=www --with-http_ssl_module --with-http_stub_status_module --with-http_realip_module --add-module=/root/ngx-fancyindex-0.5.2/ --add-module=/root/echo-nginx-module-0.63
...
location / {
    fancyindex on;
	#显示更为可读的文件大小
	fancyindex_exact_size off; 以可视化的输出
    index index;====找不到就会显示下面所有的页面
}
```
...
> 案例:
>
> ![Snipaste_2025-09-30_16-23-09](assets/Snipaste_2025-09-30_16-23-09.png)
...
>
> 注意：脚本中===> :42,49 m 92  脚本中输入指的是42-49行移动到92行。
...
以上方案就是==seo优化伪静态==的使用，把真实的后端的页面，伪装为静态html页面，也可以实现隐藏页面的效果。
...
注意新域名www.shop.com 记得在客户端的hosts文件中解析。永久重定向，适用于域名更换的场景。而且一旦某个域名301重定向到其他页面，老域名就会被搜索引擎所屏蔽。
...
server {
      ...
      root html/devops;  ===站点根目录里面有landscapr.png和default.png两张图片。
      location / {
         index index.html index.htm;
     }
     # 开启防盗链
...
server {
        listen       80;
        server_name  192.168.66.8;   ===其他公司
        root html;
        location / {
            index  index.html index.htm index.php;
        }

shell > cd /usr/local/nginx/html/test.html
shell >  <img src="http://www.devops.com/landscape.png" />
浏览器中访问：192.168.66.8/test.html回返回default.png这张图片
```
...
```nginx
#将以下配置加入到http段配置中
server_tokens off  写在http中
```

>curl http://192.168.66.110,发起http请求，得到响应内容。
>
>或者
>
>curl -I http://192.168.66.110 发起http请求，不获取响应的内容，而获取响应信息
...
mysql采集: mysqld_exporter

nginx采集： nginx_exporter

````
mkdir /usr/local/nginx-exporter
tar -xf nginx-prometheus... -C /usr/local/nginx-exporter
nohup /usr/local/nginx-exporter/nginx-prometheus-exporter --nginx.scrape-uri=http://192.168.66.8/stub_status & ===> 后台运行
启动后会产生nohup.out里面查看端口9113

浏览器中查看：192.168.66.8:9113/metrices

````

```
面试的时候从prometheus找出至少三个指标作为背诵：
Processed connections 当前连接
Active Connection 有效连接
系统连接中：CPU busy CPU的忙碌率
sysLoad 系统负载
RAM Used 内存使用情况
SWAP Used 交换内存使用情况
Root FS Used 磁盘空间
...
Uptime 时间等
监控mysql时：
MySQL UPtime MySQL运行时间
Current QPS 当前QPS
InnoDB buffur pool size 缓存池大小
```

>小结：prometheus不仅可以实现系统监控，还可以实现（应用）健康
>
>应用监控包括mysql、nginx
...
```
内网面板地址:     https://192.168.66.9:37382/55a3aef8
 username: suqhk3fv
 password: cd52b7d9
```
...
```
w网站文件目录： cd /www/wwwroot/
数据目录： cd /www/server/data/
如果无法访问宝塔面板，可能是防火墙未开放对应端口，以下命令可开放宝塔默认端口（8888）
centos 防火墙：firewall-cmd --zone=public --add-port=8888/tcp --permanent firewall-cmd --reload
```
...
![Snipaste_2025-10-01_17-13-36](assets/Snipaste_2025-10-01_17-13-36.png)

上传文件，删除原有的index.html文件

![Snipaste_2025-10-01_17-15-42](assets/Snipaste_2025-10-01_17-15-42.png)

指定项目目录为niushop文件夹

![Snipaste_2025-10-01_17-19-50](assets/Snipaste_2025-10-01_17-19-50.png)

开启PHP程序的fileinfo扩展（php-mysqlnd、php-fileinfo）

> php语言有一个概念叫做扩展，类似于java环境的jar包

![Snipaste_2025-10-01_17-23-03](assets/Snipaste_2025-10-01_17-23-03.png)

进入配置修改： 调整php脚本内存限制为300M以上

![Snipaste_2025-10-01_17-25-31](assets/Snipaste_2025-10-01_17-25-31.png)

#### 安装Niushop源码

1、win中修改hosts文件：win+r 输入drivers：192.168.66.9 www.shop.com

![Snipaste_2025-10-01_17-30-23](assets/Snipaste_2025-10-01_17-30-23.png)

![Snipaste_2025-10-01_17-31-03](assets/Snipaste_2025-10-01_17-31-03.png)

2、设置伪静态

![Snipaste_2025-10-01_17-33-43](assets/Snipaste_2025-10-01_17-33-43.png)

![Snipaste_2025-10-01_17-34-24](assets/Snipaste_2025-10-01_17-34-24.png)
...
```
vi /ysr/local/nginx/conf/nginx.conf
...
server {
   ...
   location = /stub_status{
     stub_status on;
     access_log off; 开启status状态后，不记录在access.log中
   }

}
重启nginx： sbin/nginx -s reload
浏览器中输入：域名/stub_status,出现nginx的相关信息。
```
...
mysql采集:mysql_exporter

nginx采集：nginx_exporter

```
mkdir /usr/local/nginx_exporter
tar -xf nginx-prometheus-export..tar.gz -C /usr/local/nginx_exporter/
nohup /usr/local/nginx-exporter/nginx-proemtheusexporter --nginx.scrape-uri=http://192.168.66.100/stub_status &
cat nohup.out 查看端口占用：9113
```

### 4、在prometheus中拉去nginx数据
...
- job_name: "prometheus"
  static_configs:
  - targets: ["192.168.66.90:9090"]   =====>prometheus服务器ip地址
- job_name: "nginx"
  static_configs:
  - targets: ["192.168.66.100:9113"]    ====>nginx服务器地址

重启prometheus：nohup /usr/local/nginx/prometheus/prometheus --config.files="/usr/local/promtheus/prometheus.yml" &
```
...
>1、系统层面的监控：cpu繁忙率（CPU busy）、系统负载（sys load）、内存使用率（RAM Used）、交换内存使用率（SWAP Used）、磁盘文件使用率（Root FS Used）、CPU核心数（CPU Co）等。
>
>2、mysql监控：mysql运行时间（mysql Uptime）、当前QPS(current QPS)、缓存池大小（innoDB Buffer Pool Size）、mysql最大连接数(mysql connections)、慢查询数量等。