...
> -USR2  平滑启动一个进程（平滑升级）
>
> -WINCH  优雅的关闭子进程（nginx有两个进程，一个master，子进程worker进程，关闭的只要是后者）
>
> -QUIT   优雅关闭主进程

>
>
>注意： systemctl 管理的nginx软件，没办法使用平滑升级，只有原生启动的nginx才可以使用平滑升级。
>
...
```shell
查看进程：ps -ef|grep nginx(会有master和worker两个进程)
shell > kill -USR2 旧主进程号（旧master）
```
...
```shell
shell > kill -WINCH 旧的主进程号（旧master）
shell > kill -QUIT 旧的主进程号（旧master）
此时已完成平滑升级浏览器中输入ip或者时候curl -I http://localhost ===版本已切换成新版本。
```
...
**查看nignx目录下的配置文件**（源码安装）

/usr/local/nginx/nginx.conf

>
>http区块代表接受所有http请求，在nginx.conf中有且仅有1个！！！
>
>1个http区块中可以有很多个server区块，每个server区块就相当于一个项目配置
>
>1个server区块中可以有多个location，没有location区块就相当于一个url连接匹配规则！！！

![Snipaste_2025-09-28_16-55-35](pict/Snipaste_2025-09-28_16-55-35.png)
...
![Snipaste_2026-04-15_10-57-31](pict/Snipaste_2026-04-15_10-57-31.png)

==案例三：配置基于IP的虚拟主机==

>多个ip访问同一套服务器。不通的IP解析到不同的vhost虚拟机中。
>
>核心：ens33绑定多个ip地址（选一个临时绑定ip或者配置文件绑定ip）。

补充：centos stream 9网卡ip与移除

```
nmclient ===>(nm networkmanager) ===> cli(client)
添加：
nmcli con mod ens33 +ipv4.addresses 192.168.66.200/24
nmcli con up ens33
查看：
nmcli device show ens33
移除：
nmcli con mod ens33 ipv4.addresses "192.168.66.100/24" #这个地方写要保留的ip地址
...
![Snipaste_2025-09-28_18-12-37](pict/Snipaste_2025-09-28_18-12-37.png)
...
```
总结：nginx有哪些优化手段：可以设置worker核心数 开启静态文件的gzip压缩 客户端缓存，这些都可以加快nginx的访问速度
...
告知浏览器获取的信息是在某个区间时间段是有效的。（中能缓存静态资源。图片等）
...
总结：如果返回的304（响应的状态码），代表网页中的图片以被缓存。
```
...
>401：授权失败，比如账号密码输入错误
>
>403： 权限不足，比如目录下找不到首页文件，权限不足都可能会引起403
>
>404： 文件、资源未找到
>
>500：服务端异常，比如服务端报错、数据库无法连接
>
>502：gateway网关错误，比如nginx转发请求到后端服务器，后端服务器异常，无法响应

#### 3.5、目录列表显示

显示文件列表，或者需要做一个下载列表

作用：仅用于开发、测试环境，主要用于调式代码

应用：如果一个项目配置完毕，一个目录下没有首页，则会显示403Forbidden。这个时我们可以使用aotoindex on， 显示所在目录下的所有文件列表。
...
作用：可以根据实际业务需求，分发代理页面到不同的解释器（动态http请求，负载均衡等采用了反向代理）
...
| **==$http_x_forwarded_for==** | 可以记录客户端IP，通过代理服务器来记录客户端的ip地址（真实客户端IP地址） |
...
作用：利用shell脚本对日志进行切割，把大的访问日志切割为一个一个小日志，方便以后存储及查看分析

```
/usr/local/nginx/中

mkdir /scripts
vi /scripts/logrotate.sh
#!/bin/bash
time=$(date +"%F-%H-%M")
mv /usr/local/nginx/logs/access.log /usr/local/nginx/logs/access-$time.log ===备份
/usr/local/nginx/sbin/nginx -s reload

crontab -e
* */6 * * * /bin/sh /scripts/logrotate.sh &> /dev/null

/bin/sh /scripts/logrotate.sh指的是：sh /scripts/logrotate.sh 以sh的绝对路径执行这个脚本
为什么备份日志文件以后，要重载nginx？ /usr/lcoal/nginx/sbin/nginx -s reload
答： 因为每次重载，系统都会产生一个新的access.log日志文件
```



## 4、扩展：GoAccess轻量级日志分析

作用： 正对Apache/nginx访问日志进行分析，优点：轻量级、开源、带图形化界面！

什么是GoAccess?

GoAccess是一款开源的且具有交互视图界面的 web日志分析工具，通过web浏览器或者linux系统的终端程序即可访问。能作为系统管理员提供快速且具有价值的http统计，并以在线可视化服务器的方式呈现。

```
安装GoAccess：
dnf install epel-releases -y
dnf install goaccess -y
```

运行goaccess分析access.log日志
...
>
>
>注意：goaccess分析的结果往往是一个html页面，如果想直接预览，建议直接把内容置放于nginx访问目录中。

1、离线分析(分析过去一段时间的访问信息)
...
COMMON 代表通用的日志格式（CLF）apache
```

2、实时分析

```
goaccess logs/access.log --log-format=COMMBINED -o /usr/local/nginx/html/report.html --real-time-html
```
