...
shell > mysqldump -uroot -p --database niushop > /root/niushop.sql
```
...
```mysql
mysql > grant all on niushop.* to 'niushop'@'192.168.66.%' identified by '123';
```

**④修改项目数据库配置文件**（master和backup）

```shell
shell > vim /usr/local/nginx/html/niushop/application/database.php
vi /www/wwwroot/www.shop.com/niushop/config/database.php
```
...
​         Keepalived软件主要是通过==VRRP协议==实现高可用功能的。
...
>
> Checkers：负责真实服务器的==健康检查healthchecking==，是keepalived最主要的功能。换言之，可以没有VRRP Stack，但健康检查healthchecking是一定要有的。
>
...
```
keepalived分为两个空间，一个内核空间和用户空间。内核空间几乎不需要人工参与，在故障发生时可以自动实现故障迁移、VIP的漂移等操作。而用户空间在配置文件中就已经配置了，核心会产生一个watch dog这样的进程，这个进程支持tcp、http、ssl等协议，不断检查服务的状态，一旦发生故障时，会基于VRRP STACK这个协议调用NETLINK来实现故障的切换。
```
...
>注意：shell脚本中删除36行以后，鼠标定位36行   **dG回车**
...
通过以上实验检测，发现当主服务器宕机不可用时，VIP会切换到备用服务器

>注意：当设置为keepalived时，当主节点宕机后又重新启动后，会发生争抢。为防止争抢在配置文件中设置

![Snipaste_2025-10-02_17-06-09](assets/Snipaste_2025-10-02_17-06-09.png)

#### 让keepalived与niushop产生联系

![Snipaste_2025-10-02_17-15-25](assets/Snipaste_2025-10-02_17-15-25.png)
...
![Snipaste_2025-10-02_17-41-27](assets/Snipaste_2025-10-02_17-41-27.png)
...
![Snipaste_2025-10-02_17-52-18](assets/Snipaste_2025-10-02_17-52-18.png)
...
![Snipaste_2025-10-02_18-15-33](assets/Snipaste_2025-10-02_18-15-33.png)
...
#本地IP
vrrp_instance VI_1{
	unicast_src_ip 192.168.17.102  ===>server01IP地址
	unicast_peer {
    #对象IP  发送vrrp包给备服务器
    	192.168.17.101  ====> server02IP地址
}
```
...
#本地IP
vrrp_instance VI_1 {
	unicast_src_ip 192.168.17.101  ===>server03IP地址
	unicast_peer {
    	#对象IP  发送vrrp包给备服务器
    	192.168.17.102   ===>server01IP地址
}
```
...
-n：以数字形式显示ip地址
vrrp:协议名称
默认只有主master才会向组播地址发起访问
```

>以上是因为云服务器没有开放组播地址（224.0.0.18）,没有办法进行组播，可以采用单播的方式进行配置



脑裂：

![Snipaste_2026-04-16_15-44-51](assets/Snipaste_2026-04-16_15-44-51.png)

![Snipaste_2026-04-16_15-46-55](assets/Snipaste_2026-04-16_15-46-55.png)

**遇到的问题**：

>在设置keepalived高可用时，防火墙未设置，导致出现脑裂，请求一会指向web01，一会指向web02，解决方法：查看网络没有出现问题、通信没有问题、采用抓包tcpdump -i ens33 vrrp -n 知道虚ip在两个节点上都同时指向组播地址224.0.0.18，设置正确的防火墙规则，重新启动keepalived后正常。