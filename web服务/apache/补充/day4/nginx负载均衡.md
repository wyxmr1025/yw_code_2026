...
**4）七层负载均衡（http/https）**  智能型负载均衡

根据虚拟的url或IP，主机接收请求，再转向（反向代理）相应的处理服务器

![Snipaste_2025-10-07_09-58-26](assets\Snipaste_2025-10-07_09-58-26.png)

>面试过程中：四层和七层负载均衡区别：
>
>1、底层实现=> 四层负载均衡基于网络层与传输层，也就是ip+port实现请求转发；七层负载基于应用层，通过URL地址请求转发
>
>2、性能不同=> 四层相当于转发器，效率更高；七层需要通过URL判断才能实现转发，需要做额外的处理。
>
>3、安全性不同=> 七层需要对用户URL请求进行判断，判断过程可以屏蔽异常请求，相对而言更加去安全
...
思考：重启后如何保证请求已经正常转发？
可以分别在web01和web02准备两个相同的位置，相同名称的测试文件(9和10这台服务器)

```
echo web01/02 > /www/wwwroot/www.shop.com/niushop/demo.html
```
...
#### 在web01和web02服务器上，打开配置文件，定制日志显示格式
...
![Snipaste_2025-10-07_11-41-25](assets\Snipaste_2025-10-07_11-41-25.png)
...
默认情况下，我们使用nginx属于开源nginx，默认不支持健康检查。

影响：



​	用户请求====> nginx负载均衡===> web01

​													      web02(异常，返回404文件未找到、500服务端错误)
...
cd nginx-1.26.2/
安装第三方模块
[root@start2 nginx-1.26.2]# ./configure --prefix=/usr/local/nginx --user=www --group=www --with-http_ssl_module --with-http_stub_status_module --with-http_realip_module --add-module=/root/nginx_upstream_check_module-0.4.0
...
http {
    upstream shop{
        ip_hash;
        server 192.168.66.9:80; web服务器地址+port
        server 192.168.66.10:80;
        
        check interval=3000 rise=2 fall=3 timeout=1000 type=http;
        check_http_send "GET / HTTP/1.0\r\n\r\n";
        check_http_expect_alive http_2xx http_3xx;  ===>期待返回的状态码
    }
...
check interval=3000
含义：检查时间间隔（ms毫秒）
每隔3000毫秒（3s）进行一次健康检查，这表示nginx会每隔3s像后端服务器发送一次健康检查请求，已确认服务器是否正常运行。

rise=2
含义：连续成功检查的次数
当连续2此健康检查成功时，nginx认为后端服务器是正常的。例如有一次检查失败，但接下来两次检查都成功，那么服务器标记为正常，并开始将流量转发到该服务器。

fall=3
含义：连续失败检查次数
解释：当连续3次健康检查失败时，nginx认为后端服务器是失败的，例如，如果连续三次检查都失败，那么nginx会将该服务器标记为失败。
````
...
修改win中的地址
192.168.66.200 www.shop.com
```

![Snipaste_2025-10-07_16-57-30](assets\Snipaste_2025-10-07_16-57-30.png)