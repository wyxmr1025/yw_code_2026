...
Ceph是一个能提供的**文件存储**,**块存储**和**对象存储**的分布式存储系统。它提供了一个**可无限伸缩的**Ceph存储集群。
...
可以说**RADOS就是ceph底层原生的数据引擎,** 但实际应用时却不直接使用它,而是分为如下4种方式来使用:
...
* **RADOSGW**是一套基于当前流行的RESTFUL协议的网关，并且兼容S3和Swift。**(一个基于桶的REST的网关，网关：就相当于接口，桶：相当于一个目录；对象存储)**
...
**RESTFUL**: RESTFUL是一种架构风格,提供了一组设计原则和约束条件,http就属于这种风格的典型应用。REST最大的几个特点为：资源、统一接口、URI和无状态。（**通过http协议就可以访问的接口**）
...
**Ceph Monitor**: 是一个监视器,监视Ceph集群状态和维护集群中的各种关系。（管理节点）
...
~~~powershell
[root@node1 ceph]# ceph-deploy new node1（主机名）
...
~~~powershell
[root@node1 ceph]# ceph-deploy mon create-initial  创建监控初始化
...
......
mon clock drift allowed = 2				# monitor间的时钟滴答数(默认0.5秒)时间允许的误差
...
~~~powershell
[root@node1 ceph]# ceph-deploy --overwrite-conf admin node1 node2 node3 配置文件同步到所有节点上面。
...
zap表示干掉磁盘上的数据,相当于格式化，===格式化node1上面/dev/sdb的数据
...
将磁盘创建为osd（创建数据盘）===将node上面的/dev/sdb盘创建为数据盘
...
以上是将单个硬盘合成一个大硬盘；osd都创建好了，那么怎么存取数据呢?
...
上面提到了RADOS也可以进行数据的存取操作, 但我们一般不直接使用它，但我们可以先用RADOS的方式来深入了解下ceph的数据存取原理。===怎样给客户端使用

```
首先最下面是三个硬盘（disk），将硬盘做成一个盘（osd盘，相当于逻辑卷的pv，标记为osd，ceph就能管理），将osd创建的大硬盘创建为池pool（相当于卷组VG），在池里又有PG（相当于lv，公司里的小组）；
```
...
* PG到OSD的映射的过程算法叫做CRUSH 算法。(一个Object需要保存三个副本，也就是需要保存在三个osd上)。===就是理解成通过CRUSH算法可以负载均衡+高可用，就是挂了一个节点不会有影响。
...
1/创建池：test_pool,指定pg数为128

~~~powershell
[root@node1 ceph]# ceph osd pool create test_pool 128 ===创建池pool。128就是PG数，PG数不要分配太多
pool 'test_pool' created
~~~

2/查看pg数量,可以使用ceph osd pool set test_pool pg_num 64这样的命令来尝试调整
...
~~~powershell
[root@node1 ceph]# rados put newfstab /etc/fstab --pool=test_pool    ===将本机的/etc/fstab上传到池里，并改名为newfstab
~~~
...
~~~powershell
[root@node1 ceph]# ceph osd pool create cephfs_pool 128  ==创建数据池
pool 'cephfs_pool' created
[root@node1 ceph]# ceph osd pool create cephfs_metadata 64  ==创建元数据池
...
~~~powershell
[root@node1 ceph]# ceph fs new cephfs cephfs_metadata cephfs_pool  ==将两个池合二为一，池的名字不能乱，前面是元数据的，后面是数据的
...
[root@client ~]# ceph osd pool delete cephfs_metadata cephfs_metadata --yes-i-really-really-mean-it  ===删除池
...
第1步: 在node1上创建rgw，创建对象存储网关，也就是打开一个接口等别人来连。
...
host_bucket = 10.1.1.11:7480/%(bucket)  ==对象存储的目录
cloudfront_host = 10.1.1.11:7480
use_https = False  ===不用加密传输
~~~
...
再建一个桶
[root@client ~]# s3cmd mb s3://test_bucket  ===桶不能重名。
上传文件到桶
