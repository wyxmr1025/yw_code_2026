...
```
filebeat装在nginx、httpd应用服务器上面收集日志并交给es（单传给es就没有发在logstash上进行过滤）或者logstash，redis主要起缓存的作用或者加 kafka消息队列+zookeeper（很少使用）。最后通过kibana进行图形化展示。 综上主要是filebeat收集日志输出到redis ----logstash---es----通过kibana展示
```
...
- type: log
  enabled: true
  paths:
    - /var/log/nginx/access.log  ===收集的nginx日志
...
1, 在redis服务器上,安装redis并启动(需要epel源，redis6379)
...
# vim /etc/redis.conf
61  bind 10.1.1.14			需要filebeat可以连接,或者改为0.0.0.0 允许filebeat可以连接

480 requirepass 123456    密码
...
~~~powershell
# redis-cli -h 10.1.1.14 -a 123456    连接redis,端口为6379，在浏览器中输入本机的ip地址刷新redis就会有记录进来
10.1.1.14:6379> keys *
1) "filebeattoredis"
...
input {   ====输入
    redis {
        host => "10.1.1.14"  redis的ip 端口 密码 库 key
        port => 6379
        password => "123456"
        db => "0"
        data_type => "list"
...
![1562233349586](图片/1.png)



![1562233397009](图片/2.png)



![1562233423982](图片/3.png)
...
filter {
	json {
		source => "message"  将日志的格式转化成json格式
	}
...
~~~shell
清空日志后默认有个空行造成inode会变，原来的inode就要重启服务或者reload一下
...
![1562253814661](图片/8.png)



![1562253904253](图片/9.png)

11, 可选择创建仪表盘,将图形加入仪表盘

![1562253962506](图片/10.png)



![1562254039162](图片/11.png)



![1562254092599](图片/12.png)



![1562254180549](图片/13.png)